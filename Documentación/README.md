# Documentación — Motor Tarifario Inteligente inDrive+

> Punto de entrada de toda la documentación del proyecto (MVP Lima Metropolitana).
> Cada tema tiene **un documento canónico**; este índice solo enlaza, no duplica contenido.

## 🎤 Presentación
- **[Presentación Final](Presentacion_Final/README.md)** — índice/guion de la defensa: arquitectura, sprints, pruebas, evidencias, gestión de cambio y conclusiones.

## 🏃 Scrum (por fase)
- [Sprint 0](Scrum/Sprints/sprint_0.md)
- [Sprint 1](Scrum/Sprints/sprint_1.md)
- [Sprint 2](Scrum/Sprints/sprint_2.md) — tiene su **propio índice interno**
- [Gestión de Cambio](Presentacion_Final/gestion_de_cambio.md)
- Retrospectivas: [Sprint 1](Scrum/Retrospectivas/retrospective.md) · [Sprint 2](Scrum/Retrospectivas/retrospective2.md)
- [Pruebas del sistema (estrategia)](Scrum/Pruebas/pruebas_sistema.md)

## 📋 Requerimientos
- [Historias de Usuario](Requerimientos/Historias_usuario.md) — **fuente canónica** de CAR + HU
- [Product Backlog](Requerimientos/Product_backlog.md)
- [Sprint Backlog](Requerimientos/sprint_backlog.md)

## 🏗️ Arquitectura
- [Arquitectura general](Aquitectura/Arquitectura_general.md) — **canónico** de arquitectura técnica + diagramas
- [Decisiones arquitectónicas (ADR)](Aquitectura/decisiones_arquitectonicas.md)
- [Stack tecnológico](Aquitectura/Stack_Tecnologico.md)

## 💵 Negocio
- [Flujo de cálculo de tarifa](Negocio/flujo_calculo_tarifa.md) — **canónico** de 7 variables + regla de pago
- [Reglas tarifarias](Negocio/reglas_tarifarias.md)
